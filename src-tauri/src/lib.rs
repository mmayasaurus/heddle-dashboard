    ...
    builder
        ...
        .setup(|app| {
            ...
            if let Some(win) = app.get_webview_window("main") {
                ...
                // and Mission Control; the Dock icon carries a persistent "DEV" badge set from the
                // frontend badge path (HED-159, platform/devBadge.ts) so it survives the unread-count
                // hook. (⌘-Tab shows the bundle icon without the Dock badge, so it is not covered.)
                #[cfg(debug_assertions)]
