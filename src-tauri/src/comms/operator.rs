...
/// Stable, broker-safe room name for a project's all-agent conversation. The project ID is a UUID
/// generated by the dashboard, so it is stable and contains only broker-safe lowercase hex and
/// hyphens. Including it avoids collisions between distinct project names with the same slug.
fn default_project_room_name(project_name: &str, project_id: &str) -> String {
    let slug = broker_safe_room_fragment(project_name);
    let project_slug = broker_safe_room_fragment(project_id);
    format!(
        "{}-{}-all",
        if slug.is_empty() { "project" } else { &slug },
        if project_slug.is_empty() { "project" } else { &project_slug }
    )
}
...
async fn provision_default_project_room_with_members(
    ...
    // `create_room` is idempotent by room name at the broker. Calling it for a persisted room too
    // closes a room that was previously opened, while concurrent first opens converge on this
    // project's stable ID-bearing name instead of racing to separate rooms.
