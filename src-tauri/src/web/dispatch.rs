...
pub fn dispatch(app: &AppCtx, cmd: &str, args: &Value, source: &str) -> Result<Value, String> {
    match cmd {
        ...
        // GUI-only: the unread commands read the desktop-only `comms` module, so they are absent
        // from the minimal `--no-default-features` server build (chat is desktop-only).
        #[cfg(feature = "gui")]
