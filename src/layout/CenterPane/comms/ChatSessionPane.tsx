...
export function ChatSessionPane({ chatTarget, hidden = false, area }: ChatSessionPaneProps) {
  ...
  if (!hasTarget) {
    return (
      <div className="comms-empty-state comms-session-pane" style={{ position: "absolute", ...area, display: hidden ? "none" : undefined }} data-testid="chat-session-empty-target">
        Chat target unavailable.
      </div>
    );
  }
  if (!poll.loaded) return <div className="comms-loading comms-session-pane" style={{ position: "absolute", ...area, display: hidden ? "none" : undefined }} data-testid="comms-loading" aria-hidden="true" />;
  if (!poll.schemaOk) {
    return (
      <div className="comms-schema-banner comms-session-pane" style={{ position: "absolute", ...area, display: hidden ? "none" : undefined }} data-testid="comms-schema-banner">
  ...
  if (poll.schemaVersion === 0) {
    return (
      <div className="comms-empty-state comms-session-pane" style={{ position: "absolute", ...area, display: hidden ? "none" : undefined }} data-testid="comms-empty-state">
  ...
  return (
    <div className="comms-session-pane" style={{ position: "absolute", ...area, display: hidden ? "none" : undefined }}>
      <ChatColumn
        poll={poll}
