...
const homes: string[] = [];
const envRepoint = {
  baseUrl: "https://repoint.example.test",
  authTokenRef: "secretref",
  service: "glm",
};
const registry = {
  claude: [
...
function dispatch(home: string, account: string) {
  return JSON.parse(fs.readFileSync(dispatchPath(home, account), "utf8"));
}

function dispatchPath(home: string, account: string) {
  return path.join(home, ".heddle", "usage", `claude-${account}.dispatch.json`);
}
...
function writeFakeHeddle(home: string, mode: "healthy" | "no-write" | "error", marker?: string) {
...
describe.skipIf(!hasPython3)("heddle-window-keeper", () => {
  ...
  it("does not remove a stale dispatch signal in --dry-run (write-free preview)", () => {
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toContain("repointed: WOULD remove stale dispatch signal (dry-run)");
  ...
  it("logs a dispatch cleanup failure without costing a native account its ping", () => {
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toContain("unable to remove stale dispatch signal for 'repointed' (");
  ...
  it("still selects and pings a native logged-in account when an env-repoint peer is skipped", () => {
    ...
    writeRegistry(home, [
      ...
      { id: "native", configDir: "~/.claude-native", loggedIn: true, envRepoint: null },
    ]);
  ...
  it("treats an empty envRepoint object as native because an empty dict is falsy in Python", () => {
    const home = mkHome();
    writeRegistry(home, [
      { id: "native-empty-repoint", configDir: "~/.claude-native-empty", loggedIn: true, envRepoint: {} },
    ]);
  ...
  it("loudly mutes duplicate live identities through delivery and deduplicates the mute", () => {
    const home = mkHome();
    writeRegistry(home, [
    ...
    const env = { HEDDLE_FLEET_POST_CMD: poster };
    ...
    fs.writeFileSync(marker, "must-not-be-reposted");
  ...
  it("records OAuth success when poll-claude writes a fresh sidecar", () => {
    const home = mkHome();
  ...
  it("backs off when poll-claude exits zero without writing usage", () => {
    const home = mkHome();
    writeRegistry(home, [{ id: "acct1", configDir: null, loggedIn: true }]);
    const result = runKeeper([], home, { HEDDLE_BIN: writeFakeHeddle(home, "no-write") });

    expect(result.status).toBe(0);
    expect(JSON.parse(fs.readFileSync(path.join(home, ".heddle", "oauth-usage-state.json"), "utf8")).attempts.acct1).toHaveProperty("lastAttemptAt");
  });

  it("backs off when poll-claude returns an error", () => {
    const home = mkHome();
    writeRegistry(home, [{ id: "acct1", configDir: null, loggedIn: true }]);
    const result = runKeeper([], home, { HEDDLE_BIN: writeFakeHeddle(home, "error") });

    expect(result.status).toBe(0);
    expect(JSON.parse(fs.readFileSync(path.join(home, ".heddle", "oauth-usage-state.json"), "utf8")).attempts.acct1).toHaveProperty("lastAttemptAt");
  });

  it("skips poll-claude while the OAuth sidecar is fresh", () => {
    const home = mkHome();
    const marker = path.join(home, "poll-claude-calls");
    writeRegistry(home, [{ id: "acct1", configDir: null, loggedIn: true }]);
    fs.writeFileSync(path.join(home, ".heddle", "usage", "claude-acct1.oauth-usage.json"), "{}");
    const result = runKeeper([], home, { HEDDLE_BIN: writeFakeHeddle(home, "healthy", marker) });
  ...
  it("skips poll-claude while an OAuth backoff is active", () => {
    const home = mkHome();
    const marker = path.join(home, "poll-claude-calls");
    writeRegistry(home, [{ id: "acct1", configDir: null, loggedIn: true }]);
    fs.writeFileSync(path.join(home, ".heddle", "oauth-usage-state.json"), JSON.stringify({ attempts: { acct1: { lastAttemptAt: Math.floor(Date.now() / 1000) } } }));
    const result = runKeeper([], home, { HEDDLE_BIN: writeFakeHeddle(home, "healthy", marker) });
  ...
  it("skips OAuth refresh when another run holds its flock", async () => {
    const home = mkHome();
    const marker = path.join(home, "oauth-lock-poll-claude");
...
`, lock, ready], { stdio: "ignore" });
    ...
    try {
      writeRegistry(home, [{ id: "acct1", configDir: null, loggedIn: true }]);
      const result = runKeeper([], home, {
        HEDDLE_BIN: writeFakeHeddle(home, "healthy", marker),
      });
