...
const ptBR: typeof en = {
  ...
  "fleet.title": "Frota", "fleet.refresh": "Atualizar", "fleet.capsWaiting": "limites: aguardando a renderização da barra de status…", "fleet.usageUnavailable": "uso indisponível", "fleet.noActiveWindow": "sem janela ativa", "fleet.capturedMinutesAgo": (n) => `capturado há ${n} min`, "fleet.stale": "desatualizado", "fleet.idle": "inativo", "fleet.idleHint": "5h/7d só são atualizados enquanto esta conta conduz uma sessão", "fleet.limitReached": "limite atingido", "fleet.resetting": "redefinindo…", "fleet.currentProject": "Projeto atual", "fleet.allAgents": "Todos os agentes", "fleet.loggedOut": "desconectado — /login necessário", "fleet.keeperEstimate": "janela ativa (estimativa do keeper) — % aparece após a primeira renderização nesta conta", "fleet.loginUnknown": "estado de login desconhecido", "fleet.rotateAccounts": "Alternar contas", "fleet.fableWeekly": (pct) => `Fable ≈${pct}% do limite semanal (est.)`, "fleet.fableWeeklyExact": (pct) => `Fable ${pct}% do limite semanal`, "fleet.fableWeeklyEstMark": "est.", "fleet.fableWeeklyBreakdown": (fable, other, unknown, samples) => `Fable ${fable}% · outros ${other}% · desconhecido ${unknown}% · ${samples} amostras`, "fleet.hudModel": (model) => `modelo ${model}`, "fleet.hudGit": (branch, dirty) => `git ${branch}${dirty ? " *" : ""}`, "fleet.hudContext": (pct) => `contexto ${pct}%`, "fleet.hudResources": (claudeMd, rules, mcp) => `CLAUDE.md ${claudeMd} · regras ${rules} · MCP ${mcp}`, "fleet.hudAge": (age) => `há ${age}`, "fleet.hudSeparator": " · ",
  ...
  "share.title": "Compartilhar o heddle", // Share heddle
  "share.subtitle":
    "Somos uma pequena equipe por trás do heddle. Se você gosta dele, compartilhe o heddle com outras pessoas. Ajudar mais gente a nos conhecer significa muito para nossa equipe. Obrigado pelo apoio! ❤️", // We're a small team behind heddle. If you enjoy it, please share heddle with others…
  ...
  "connect.desc":
    "Digite o endereço e a senha de um heddle remoto para conectar e controlá-lo em uma nova janela.", // Enter the address and password…
  ...
  "agentInstall.desc": (label) =>
    `O heddle não encontrou ${label} no seu PATH. Instale-o para iniciar esta sessão.`, // couldn't find {label} on PATH
  ...
  "fleet.comms.targetUnavailable": "O destino do chat não está disponível.",
  ...
  "fleet.comms.deliveredNoLiveSession": (target) => `Entregue ao registro — ${target} não tem uma sessão ativa, então não receberá um aviso em tempo real (a mensagem ainda pode ser consultada).`, // Delivered to the log — {target} has no live session…
  "fleet.comms.broadcastInboxSplit": (inbox, total) => `${inbox} de ${total} destinatários não têm sessão ativa — entregue às suas caixas de entrada.`, // {inbox} of {total} recipients have no live session…
