...
const zhTW: typeof en = {
  ...
  "fleet.title": "叢集", "fleet.refresh": "重新整理", "fleet.capsWaiting": "限額：等待狀態列轉譯…", "fleet.usageUnavailable": "無法取得使用量", "fleet.noActiveWindow": "沒有作用中視窗", "fleet.capturedMinutesAgo": (n) => `${n} 分鐘前擷取`, "fleet.stale": "已過期", "fleet.idle": "閒置", "fleet.idleHint": "5h/7d 僅在此帳號驅動工作階段時更新", "fleet.limitReached": "已達限額", "fleet.resetting": "正在重設…", "fleet.currentProject": "目前專案", "fleet.allAgents": "所有智能體", "fleet.loggedOut": "已登出 — 需要 /login", "fleet.keeperEstimate": "視窗仍有效（keeper 估算）— 此帳號首次轉譯後顯示 %", "fleet.loginUnknown": "登入狀態未知", "fleet.rotateAccounts": "切換帳號", "fleet.fableWeekly": (pct) => `Fable 約占每週限額的 ${pct}%（估算）`, "fleet.fableWeeklyExact": (pct) => `Fable 占每週限額的 ${pct}%`, "fleet.fableWeeklyEstMark": "估算", "fleet.fableWeeklyBreakdown": (fable, other, unknown, samples) => `Fable ${fable}% · 其他 ${other}% · 未知 ${unknown}% · ${samples} 個樣本`, "fleet.hudModel": (model) => `模型 ${model}`, "fleet.hudGit": (branch, dirty) => `git ${branch}${dirty ? " *" : ""}`, "fleet.hudContext": (pct) => `上下文 ${pct}%`, "fleet.hudResources": (claudeMd, rules, mcp) => `CLAUDE.md ${claudeMd} · 規則 ${rules} · MCP ${mcp}`, "fleet.hudAge": (age) => `${age}前`, "fleet.hudSeparator": " · ",
  ...
  "fleet.comms.targetUnavailable": "聊天目標無法使用。",
