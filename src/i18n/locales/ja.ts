...
const ja: typeof en = {
  ...
  "fleet.title": "フリート", "fleet.refresh": "更新", "fleet.capsWaiting": "上限: ステータスラインの描画待ち…", "fleet.usageUnavailable": "使用量を取得できません", "fleet.noActiveWindow": "有効なウィンドウなし", "fleet.capturedMinutesAgo": (n) => `${n}分前に取得`, "fleet.stale": "古い", "fleet.idle": "アイドル", "fleet.idleHint": "5時間/7日間は、このアカウントがセッションを操作している間のみ更新されます", "fleet.limitReached": "上限到達", "fleet.resetting": "リセット中…", "fleet.currentProject": "現在のプロジェクト", "fleet.allAgents": "すべてのエージェント", "fleet.loggedOut": "ログアウト中 — /login が必要", "fleet.keeperEstimate": "ウィンドウは有効（keeper 推定）— このアカウントの初回描画後に % を表示", "fleet.loginUnknown": "ログイン状態不明", "fleet.rotateAccounts": "アカウントを切り替える", "fleet.fableWeekly": (pct) => `Fable は週間上限の約${pct}%（推定）`, "fleet.fableWeeklyExact": (pct) => `Fable は週間上限の${pct}%`, "fleet.fableWeeklyEstMark": "推定", "fleet.fableWeeklyBreakdown": (fable, other, unknown, samples) => `Fable ${fable}% · その他 ${other}% · 不明 ${unknown}% · ${samples} 件のサンプル`, "fleet.hudModel": (model) => `モデル ${model}`, "fleet.hudGit": (branch, dirty) => `git ${branch}${dirty ? " *" : ""}`, "fleet.hudContext": (pct) => `コンテキスト ${pct}%`, "fleet.hudResources": (claudeMd, rules, mcp) => `CLAUDE.md ${claudeMd} · ルール ${rules} · MCP ${mcp}`, "fleet.hudAge": (age) => `${age}前`, "fleet.hudSeparator": " · ",
  ...
  "fleet.comms.targetUnavailable": "チャットの宛先を利用できません。",
