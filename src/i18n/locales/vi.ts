...
const vi: typeof en = {
  ...
  "fleet.title": "Đội", "fleet.refresh": "Làm mới", "fleet.capsWaiting": "giới hạn: đang chờ thanh trạng thái hiển thị…", "fleet.usageUnavailable": "không có dữ liệu sử dụng", "fleet.noActiveWindow": "không có cửa sổ hoạt động", "fleet.capturedMinutesAgo": (n) => `đã ghi nhận ${n} phút trước`, "fleet.stale": "cũ", "fleet.idle": "không hoạt động", "fleet.idleHint": "5h/7d chỉ làm mới khi tài khoản này điều khiển một phiên", "fleet.limitReached": "đã đạt giới hạn", "fleet.resetting": "đang đặt lại…", "fleet.currentProject": "Dự án hiện tại", "fleet.allAgents": "Tất cả tác nhân", "fleet.loggedOut": "đã đăng xuất — cần /login", "fleet.keeperEstimate": "cửa sổ đang hoạt động (ước tính keeper) — % xuất hiện sau lần hiển thị đầu tiên của tài khoản này", "fleet.loginUnknown": "không rõ trạng thái đăng nhập", "fleet.rotateAccounts": "Chuyển tài khoản", "fleet.fableWeekly": (pct) => `Fable ≈${pct}% giới hạn tuần (ước tính)`, "fleet.fableWeeklyExact": (pct) => `Fable ${pct}% giới hạn tuần`, "fleet.fableWeeklyEstMark": "ước tính", "fleet.fableWeeklyBreakdown": (fable, other, unknown, samples) => `Fable ${fable}% · khác ${other}% · không rõ ${unknown}% · ${samples} mẫu`, "fleet.hudModel": (model) => `mô hình ${model}`, "fleet.hudGit": (branch, dirty) => `git ${branch}${dirty ? " *" : ""}`, "fleet.hudContext": (pct) => `ngữ cảnh ${pct}%`, "fleet.hudResources": (claudeMd, rules, mcp) => `CLAUDE.md ${claudeMd} · quy tắc ${rules} · MCP ${mcp}`, "fleet.hudAge": (age) => `${age} trước`, "fleet.hudSeparator": " · ",
  ...
  "fleet.comms.targetUnavailable": "Mục tiêu trò chuyện không khả dụng.",
