...
const zhCN: typeof en = {
  ...
  "common.retry": "重试",
  "common.refresh": "刷新",
  "fleet.title": "集群", "fleet.refresh": "刷新", "fleet.capsWaiting": "限额：等待状态栏渲染…", "fleet.usageUnavailable": "使用量不可用", "fleet.noActiveWindow": "无活动窗口", "fleet.capturedMinutesAgo": (n) => `${n} 分钟前捕获`, "fleet.stale": "已过期", "fleet.idle": "空闲", "fleet.idleHint": "5h/7d 仅在此账号驱动会话时刷新", "fleet.limitReached": "已达限额", "fleet.resetting": "正在重置…", "fleet.currentProject": "当前项目", "fleet.allAgents": "所有智能体", "fleet.loggedOut": "已登出 — 需要 /login", "fleet.keeperEstimate": "窗口仍有效（keeper 估算）— 此账号首次渲染后显示 %", "fleet.loginUnknown": "登录状态未知", "fleet.rotateAccounts": "切换账号", "fleet.fableWeekly": (pct) => `Fable 约占每周限额的 ${pct}%（估算）`, "fleet.fableWeeklyExact": (pct) => `Fable 占每周限额的 ${pct}%`, "fleet.fableWeeklyEstMark": "估算", "fleet.fableWeeklyBreakdown": (fable, other, unknown, samples) => `Fable ${fable}% · 其他 ${other}% · 未知 ${unknown}% · ${samples} 个样本`, "fleet.hudModel": (model) => `模型 ${model}`, "fleet.hudGit": (branch, dirty) => `git ${branch}${dirty ? " *" : ""}`, "fleet.hudContext": (pct) => `上下文 ${pct}%`, "fleet.hudResources": (claudeMd, rules, mcp) => `CLAUDE.md ${claudeMd} · 规则 ${rules} · MCP ${mcp}`, "fleet.hudAge": (age) => `${age}前`, "fleet.hudSeparator": " · ",
  "common.loading": "加载中…",
  "common.prev": "上一个",
  "common.next": "下一个",
  "common.on": "开",
  ...
  "fleet.comms.deliveredNoLiveSession": (target) => `已发送到日志 — ${target} 没有实时会话，因此不会收到实时提醒（仍可拉取该消息）。`, // Delivered to the log — {target} has no live session…
  "fleet.comms.broadcastInboxSplit": (inbox, total) => `${total} 位收件人中有 ${inbox} 位没有实时会话 — 已发送到他们的收件箱。`, // {inbox} of {total} recipients have no live session…
