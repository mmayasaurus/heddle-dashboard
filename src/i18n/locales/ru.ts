...
const ru: typeof en = {
  ...
  "fleet.title": "Флот", "fleet.refresh": "Обновить", "fleet.capsWaiting": "лимиты: ожидание отрисовки строки состояния…", "fleet.usageUnavailable": "использование недоступно", "fleet.noActiveWindow": "нет активного окна", "fleet.capturedMinutesAgo": (n) => `зафиксировано ${n} ${plural(n, "минуту", "минуты", "минут")} назад`, "fleet.stale": "устарело", "fleet.idle": "неактивен", "fleet.idleHint": "Данные за 5 ч/7 дн. обновляются только пока этот аккаунт ведёт сеанс", "fleet.limitReached": "лимит исчерпан", "fleet.resetting": "сброс…", "fleet.currentProject": "Текущий проект", "fleet.allAgents": "Все агенты", "fleet.loggedOut": "выход выполнен — нужен /login", "fleet.keeperEstimate": "окно активно (оценка keeper) — % появится после первого рендера этого аккаунта", "fleet.loginUnknown": "статус входа неизвестен", "fleet.rotateAccounts": "Переключить аккаунты", "fleet.fableWeekly": (pct) => `Fable ≈${pct}% недельного лимита (оценка)`, "fleet.fableWeeklyExact": (pct) => `Fable ${pct}% недельного лимита`, "fleet.fableWeeklyEstMark": "оцен.", "fleet.fableWeeklyBreakdown": (fable, other, unknown, samples) => `Fable ${fable}% · другое ${other}% · неизвестно ${unknown}% · ${samples} ${plural(samples, "образец", "образца", "образцов")}`, "fleet.hudModel": (model) => `модель ${model}`, "fleet.hudGit": (branch, dirty) => `git ${branch}${dirty ? " *" : ""}`, "fleet.hudContext": (pct) => `контекст ${pct}%`, "fleet.hudResources": (claudeMd, rules, mcp) => `CLAUDE.md ${claudeMd} · правила ${rules} · MCP ${mcp}`, "fleet.hudAge": (age) => `${age} назад`, "fleet.hudSeparator": " · ",
  ...
  "share.title": "Поделиться heddle", // Share heddle
  "share.subtitle":
    "heddle создаёт небольшая команда. Если вам нравится продукт, поделитесь им с другими. Для нас очень важно, чтобы больше людей узнали о heddle и о нашей команде. Спасибо за поддержку! ❤️", // We're a small team behind heddle. If you enjoy it, please share heddle with others…
  ...
  "connect.desc":
    "Введите адрес и пароль удалённого heddle, чтобы подключиться и управлять им в новом окне.", // Enter the address and password…
  ...
  "agentInstall.desc": (label) =>
    `heddle не нашёл ${label} в PATH. Установите его, чтобы запустить эту сессию.`, // couldn't find {label} on PATH
  ...
  "fleet.comms.targetUnavailable": "Цель чата недоступна.",
  ...
  "fleet.comms.deliveredNoLiveSession": (target) => `Доставлено в журнал — у ${target} нет активной сессии, поэтому уведомление в реальном времени не придёт (сообщение всё ещё можно получить).`, // Delivered to the log — {target} has no live session…
  "fleet.comms.broadcastInboxSplit": (inbox, total) => `У ${inbox} из ${total} получателей нет активной сессии — доставлено в их входящие.`, // {inbox} of {total} recipients have no live session…
