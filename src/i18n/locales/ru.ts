...
const ru: typeof en = {
  ...
  "fleet.title": "Флот", "fleet.refresh": "Обновить", "fleet.capsWaiting": "лимиты: ожидание отрисовки строки состояния…", "fleet.usageUnavailable": "использование недоступно", "fleet.noActiveWindow": "нет активного окна", "fleet.capturedMinutesAgo": (n) => `зафиксировано ${n} ${plural(n, "минуту", "минуты", "минут")} назад`, "fleet.stale": "устарело", "fleet.idle": "неактивен", "fleet.idleHint": "Данные за 5 ч/7 дн. обновляются только пока этот аккаунт ведёт сеанс", "fleet.limitReached": "лимит исчерпан", "fleet.resetting": "сброс…", "fleet.currentProject": "Текущий проект", "fleet.allAgents": "Все агенты", "fleet.loggedOut": "выход выполнен — нужен /login", "fleet.keeperEstimate": "окно активно (оценка keeper) — % появится после первого рендера этого аккаунта", "fleet.loginUnknown": "статус входа неизвестен", "fleet.rotateAccounts": "Переключить аккаунты", "fleet.fableWeekly": (pct) => `Fable ≈${pct}% недельного лимита (оценка)`, "fleet.fableWeeklyExact": (pct) => `Fable ${pct}% недельного лимита`, "fleet.fableWeeklyEstMark": "оцен.", "fleet.fableWeeklyBreakdown": (fable, other, unknown, samples) => `Fable ${fable}% · другое ${other}% · неизвестно ${unknown}% · ${samples} ${plural(samples, "образец", "образца", "образцов")}`,
