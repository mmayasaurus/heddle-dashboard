...
const es: typeof en = {
  ...
  "fleet.title": "Flota", "fleet.refresh": "Actualizar", "fleet.capsWaiting": "límites: esperando el renderizado de la barra de estado…", "fleet.usageUnavailable": "uso no disponible", "fleet.noActiveWindow": "sin ventana activa", "fleet.capturedMinutesAgo": (n) => `capturado hace ${n} min`, "fleet.stale": "desactualizado", "fleet.idle": "inactivo", "fleet.idleHint": "5h/7d solo se actualizan mientras esta cuenta dirige una sesión", "fleet.limitReached": "límite alcanzado", "fleet.resetting": "restableciendo…", "fleet.currentProject": "Proyecto actual", "fleet.allAgents": "Todos los agentes", "fleet.loggedOut": "sesión cerrada — se necesita /login", "fleet.keeperEstimate": "ventana activa (estimación del keeper) — el % aparece tras el primer renderizado de esta cuenta", "fleet.loginUnknown": "estado de inicio de sesión desconocido", "fleet.rotateAccounts": "Rotar cuentas", "fleet.fableWeekly": (pct) => `Fable ≈${pct}% del límite semanal (est.)`, "fleet.fableWeeklyExact": (pct) => `Fable ${pct}% del límite semanal`, "fleet.fableWeeklyEstMark": "est.", "fleet.fableWeeklyBreakdown": (fable, other, unknown, samples) => `Fable ${fable}% · otro ${other}% · desconocido ${unknown}% · ${samples} muestras`, "fleet.hudModel": (model) => `modelo ${model}`, "fleet.hudGit": (branch, dirty) => `git ${branch}${dirty ? " *" : ""}`, "fleet.hudContext": (pct) => `contexto ${pct}%`, "fleet.hudResources": (claudeMd, rules, mcp) => `CLAUDE.md ${claudeMd} · reglas ${rules} · MCP ${mcp}`, "fleet.hudAge": (age) => `hace ${age}`, "fleet.hudSeparator": " · ",
  ...
  "settings.cliInstall": "Instalar el comando ‘heddle’",
  "settings.cliUninstall": "Desinstalar el comando ‘heddle’",
  "settings.cliInstalledAt": (path: string) => `Instalado en ${path}`,
  "settings.cliConflict": (path: string) =>
    `Ya existe otro comando ‘heddle’ en ${path}. heddle no lo sobrescribirá.`,
  "settings.cliHint": "Añade `heddle <ruta-del-proyecto>` al PATH, como el comando `code` de VS Code.",
  ...
  "fleet.comms.targetUnavailable": "El destino del chat no está disponible.",
  ...
  "fleet.comms.deliveredNoLiveSession": (target) => `Entregado al registro — ${target} no tiene sesión activa, así que no recibirá un aviso en vivo (aún puede consultar el mensaje).`, // Delivered to the log — {target} has no live session…
  "fleet.comms.broadcastInboxSplit": (inbox, total) => `${inbox} de ${total} destinatarios no tienen sesión activa — entregado a su bandeja de entrada.`, // {inbox} of {total} recipients have no live session…
