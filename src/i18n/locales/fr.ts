...
const fr: typeof en = {
  ...
  "fleet.title": "Flotte", "fleet.refresh": "Actualiser", "fleet.capsWaiting": "plafonds : en attente d’un rendu de barre d’état…", "fleet.usageUnavailable": "utilisation indisponible", "fleet.noActiveWindow": "aucune fenêtre active", "fleet.capturedMinutesAgo": (n) => `capturé il y a ${n} min`, "fleet.stale": "obsolète", "fleet.idle": "inactif", "fleet.idleHint": "Les données 5 h/7 j ne s’actualisent que lorsque ce compte pilote une session", "fleet.limitReached": "plafond atteint", "fleet.resetting": "réinitialisation…", "fleet.currentProject": "Projet actuel", "fleet.allAgents": "Tous les agents", "fleet.loggedOut": "déconnecté — /login requis", "fleet.keeperEstimate": "fenêtre active (estimation keeper) — le % apparaît après le premier rendu de ce compte", "fleet.loginUnknown": "état de connexion inconnu", "fleet.rotateAccounts": "Faire tourner les comptes", "fleet.fableWeekly": (pct) => `Fable ≈${pct}% du plafond hebdomadaire (est.)`, "fleet.fableWeeklyExact": (pct) => `Fable ${pct}% du plafond hebdomadaire`, "fleet.fableWeeklyEstMark": "est.", "fleet.fableWeeklyBreakdown": (fable, other, unknown, samples) => `Fable ${fable}% · autre ${other}% · inconnu ${unknown}% · ${samples} échantillons`, "fleet.hudModel": (model) => `modèle ${model}`, "fleet.hudGit": (branch, dirty) => `git ${branch}${dirty ? " *" : ""}`, "fleet.hudContext": (pct) => `contexte ${pct}%`, "fleet.hudResources": (claudeMd, rules, mcp) => `CLAUDE.md ${claudeMd} · règles ${rules} · MCP ${mcp}`, "fleet.hudAge": (age) => `il y a ${age}`, "fleet.hudSeparator": " · ",
  ...
  "titlebar.browser": "Navigateur intégré (⌘⇧B)", // Built-in Browser (⌘⇧B)
  ...
  "share.title": "Partager heddle", // Share heddle
  "share.subtitle":
    "Nous sommes une petite équipe derrière heddle. Si vous l’appréciez, partagez heddle autour de vous. Nous aider à nous faire connaître compte énormément pour notre équipe. Merci pour votre soutien ! ❤️", // We're a small team behind heddle. If you enjoy it, please share heddle with others…
  ...
  "connect.desc":
    "Saisissez l'adresse et le mot de passe d'un heddle distant pour vous y connecter et le contrôler dans une nouvelle fenêtre.", // Enter the address and password…
  ...
  "tree.newRoom": "Nouveau salon", // New room
  ...
  "tree.globalSearch": "Rechercher dans toutes les sessions (⌘⇧F)", // Search All Sessions (⌘⇧F)
  ...
  "agentInstall.desc": (label) =>
    `heddle n'a pas trouvé ${label} dans votre PATH. Installez-le pour lancer cette session.`, // couldn't find {label} on PATH
  ...
  "doc.saveTooltip": "Enregistrer (⌘S)", // Save (⌘S)
  "doc.externalChanged":
  ...
  "fleet.comms.targetUnavailable": "La cible de discussion n’est pas disponible.",
  ...
  "fleet.comms.deliveredNoLiveSession": (target) => `Enregistré dans le journal — ${target} n’a pas de session active, donc ne recevra pas de notification en direct (le message reste accessible).`, // Delivered to the log — {target} has no live session…
  "fleet.comms.broadcastInboxSplit": (inbox, total) => `${inbox} destinataires sur ${total} n’ont pas de session active — livré dans leur boîte de réception.`, // {inbox} of {total} recipients have no live session…
