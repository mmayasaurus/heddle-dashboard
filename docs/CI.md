...
| `.deepsource.toml` | dormant — DeepSource was **removed** in the 2026-08-21 bot purge (was HED-13) | JS/TS (React) + Rust + shell + secrets analyzers, repo-accurate config | config retained but inert (no analyzer runs); removable under a follow-up |
...
was written to pre-configure DeepSource (an unconfigured DeepSource once opened ~118 non-defect threads on one Spinventory PR); DeepSource was removed in the 2026-08-21 purge, so the config now sits dormant.
