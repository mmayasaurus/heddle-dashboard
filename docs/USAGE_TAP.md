...
- **Window-keeper** (`scripts/heddle-window-keeper.py`, installed at `~/.heddle/window-keeper.py`,
  ...
  account's clock. Maintenance is native-only: env-repoint accounts routed through
  `ANTHROPIC_BASE_URL` are dispatch-only and are excluded from keeper pings, rotation advice,
  transcript accounting, and OAuth polling. For an EXPIRED/UNKNOWN native account with live
  peers, the keeper schedules its fresh reset at the midpoint of the widest gap in their 5h reset
  phases, gradually spreading the fleet
